...
class ModelArguments:
    ...
    use_moh: Optional[bool] = field(default=False) # use for MoH models
    shared_heads: Optional[float] = field(default=0.5) # fraction of number of heads that are shared for MoH models
    routed_heads: Optional[float] = field(default=0.25)  # fraction of number of heads that are routed for MoH models
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    use_load_balance_loss: Optional[bool] = field(default=False)
...
def train(attn_implementation=None):
    ...
    moh_model_args = {}
    if model_args.use_moh:
        moh_model_args.update(dict(
            use_moh=model_args.use_moh,
            shared_heads=model_args.shared_heads,
            routed_heads=model_args.routed_heads,
            use_load_balance_loss=training_args.use_load_balance_loss,
        ))
    ...
    # Train router for MoH
    if model_args.use_moh:
        for name, param in model.named_parameters():
            # if 'o_proj' in name:
            #     param.requires_grad = True
            #     print("Setting gradients = True for", name)
            if model_args.shared_heads == 0:
                if 'wg_routed' in name:
                    param.requires_grad = True
                    print("Setting gradients = True for", name)
            else:
                if 'wg_' in name:
