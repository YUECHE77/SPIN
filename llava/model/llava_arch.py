...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, outlier_tokens=False):
        image_features = self.get_model().get_vision_tower()(images, outlier_tokens)
        image_features = self.get_model().mm_projector(image_features)
        return image_features

    def prepare_inputs_labels_for_multimodal(
        self, input_ids, attention_mask, past_key_values, labels, images, outlier_tokens=False
    ):
        token_ids = None
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features = self.encode_images(images, outlier_tokens)
