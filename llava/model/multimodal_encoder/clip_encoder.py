...
class CLIPVisionTower(nn.Module):
    ...
    def feature_select(self, image_forward_outs, keep_indices=None):
        image_features = image_forward_outs.hidden_states[self.select_layer]
        if self.select_feature == 'patch':
            image_features = image_features[:, 1:]

            if keep_indices is not None:  # [DAMRO]
                image_features = image_features[:, keep_indices, :]
    ...
    @torch.no_grad()
    def forward(self, images, outlier_tokens=False):
        keep_indices = None
        if type(images) is list:
            ...
            if is_internvl_14b_model(self.vision_tower_name):
                ...
                # [DAMRO]
                if outlier_tokens:
                    # Get attention weights from cls token to other tokens averaged over all heads
                    attn_avg = image_forward_outs.attentions[-1][0, :, 0, 1:].sum(dim=0)
                    top_index = torch.topk(attn_avg, k=10)[1]  # outlier_topk = 10
                    keep_indices = top_index

                image_features = self.feature_select(image_forward_outs, keep_indices).to(images.dtype)
