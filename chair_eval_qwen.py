...
parser = argparse.ArgumentParser(description="CHAIR evaluation on Qwen-VL-Chat.")
...
# ------------------- OPERA -------------------
parser.add_argument("--use-opera", action="store_true")
...
# ------------------- VCD -------------------
parser.add_argument("--use-cd", action='store_true')
...
coco_dataset = COCODataSet(data_path=args.data_path, trans=image_processor)
coco_loader = torch.utils.data.DataLoader(
    coco_dataset, batch_size=args.batch_size, shuffle=False, num_workers=32
)
# ------------------------------------------------------------

CHAIR_QUESTION = 'Describe this image in detail.'
for batch_id, data in tqdm(enumerate(coco_loader), total=len(coco_loader)):
    ...
    if args.output_path:
        with open(args.output_path, "a", encoding='utf-8') as f:
            json.dump({"image_id": int(img_id[0]), "caption": response}, f)
            f.write("\n")
