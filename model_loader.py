...
def load_llava_model(model_path, **kwargs):
...
class ModelLoader:
    def __init__(self, model_name, llava_size):
        self.model_name = model_name
        self.llava_size = llava_size
        self.tokenizer = None
        self.vlm_model = None
        self.llm_model = None
    ...
    def load_model(self):
        if self.model_name == "llava-1.5":
            if self.llava_size == '7b':
                model_path = os.path.expanduser("/path/to/llava-v1.5-7b")
            elif self.llava_size == '13b':
                model_path = os.path.expanduser("/path/to/llava-v1.5-13b")
            else:
                raise ValueError(f"llava_size must either be 7b or 13b. But your llava_size is: {self.llava_size}")
    ...
    def prepare_inputs_for_model(self, template, query, image):
        if self.model_name == "llava-1.5":
            ...
            # For OPERA:
            self.response_start_idx = kwargs["input_ids"].shape[1] + IMAGE_TOKEN_LENGTH - 1
            ...
            # For OPEAR:
            self.response_start_idx = kwargs['inputs_embeds'].shape[1]
            ...
            # For OPERA:
            self.response_start_idx = kwargs["input_ids"].shape[1]
    ...
    def init_cfg_processor(self, questions, gamma=1.1, beam=1, start_layer=0, end_layer=32):
        ...
        # replaces the LogitsProcessor in transformers
